...
if "%PKG_NAME%"=="scipy" (
    ...
    %PYTHON% -c "from pathlib import Path; p=Path('pyproject.toml'); text=p.read_text(encoding='utf-8'); old='name = '+chr(34)+'scipy'+chr(34); assert text.count(old)==1; p.write_text(text.replace(old, old.replace('scipy','scipy-tests')), encoding='utf-8')"
